...
// ---- スプリント (ダッシュ) ----
// 通常の移動速度 (RUN_SPEED / DRIBBLE_SPEED) は「ジョグ」の速さ。
// Shift (タッチはスティックを外周まで倒す) で MULT 倍に加速できるが、
// スタミナを一気に消費する。守備AIのチェイサーも同じ仕組みで加速するため、
// 「スタミナ切れの相手なら振り切れる」という駆け引きが生まれる
const SPRINT_CONF = {
  MULT: 1.18,          // スプリント中の速度倍率
  STICK_RATIO: 0.85,   // タッチ: スティックをこの比率より外まで倒すとスプリント
};

// ---- スタミナ (疲労) ----
// スプリント中に大きく減り、動きを緩めれば回復する。
// スタミナが尽きると速度が落ち、スプリント自体もできなくなる。
// ハーフタイムでも少しだけ回復する (完全回復はしない)
const STAMINA_CONF = {
  MAX: 100,
  SPRINT_DRAIN: 7.0,   // スプリント中の毎秒消費量 (満タンから約14秒)
  MOVE_DRAIN: 0.7,     // 通常移動 (ジョグ) 中のわずかな消費量
  RECOVER_RATE: 9.0,   // ほぼ静止しているときの毎秒回復量
  MIN_MULT: 0.82,      // スタミナ0のときの速度倍率 (最大で18%低下)
  SPRINT_MIN: 12,      // これ未満ではスプリントを開始できない (息切れ)
  HALFTIME_RECOVER: 45,
};
...
// ---- 1対1 (ブレイクアウェイ) ----
// GK と1対1で抜け出した瞬間、PK と同じコース読み合いのミニゲームに入る。
// 発動条件: 相手ペナルティエリア手前まで持ち込み、後ろから追う相手DFが
// CHASE_GAP 以上離れていること。1試合あたりの発動回数を抑えて特別感を残す
const BREAKAWAY_CONF = {
  TRIGGER_DIST: 26,     // 相手ゴールからこの距離以内で発動判定
  CHASE_GAP: 7,         // 最も近い相手フィールド選手がこれ以上離れていること
  COOLDOWN: 25,         // 一度発動したら次まで空ける時間(秒)
  AIM_TIME: 2.2,        // コースを選べる時間 (PKより短くスリリングに)
  ANIM_TIME: 1.2,
  SETUP_TIME: 0.9,
};
...
const FX_CONF = {
  ...
  ZOOM_BASE: 12,        // 通常時の1mあたりピクセル数 (従来の固定値)
  ZOOM_MAX: 15.5,       // ゴール前まで攻め込んだときの最大ズーム
  ZOOM_NEAR: 26,        // ゴールからこの距離以内で最大ズームになる
  ZOOM_FAR: 55,         // ゴールからこの距離以上で通常ズーム
...
const SWITCH_CONF = {
  ...
  // ボールより敵陣側 (前) に少しでも出ている選手には、まずこの固定
  // ペナルティを科す (単純な距離の近さだけでは選ばれないようにするため)。
  // そこに 1m 出ているごとに追加でこのペナルティも加算する。
  // 左右にどれだけずれていても、ボールより前の選手が選ばれにくくなる
  AHEAD_BASE_PENALTY: 25,
  AHEAD_PENALTY: 3.2,
